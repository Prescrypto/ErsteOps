...
(function(global, factory) {
  ...
  function on() {
    ...
    let [targets, types, selector, listener, useCapture = false] = getArgs(
      args
    );
  ...
  function offsetViewport(scrollElement) {
    ...
    for (let [prop, dir, start, end] of [
      ...
      rect[prop] = rect[dir] = (isWindow(viewportElement)
  ...
  function matchFilter(el, attr, _ref3) {
    ...
    const { filter = '', group = '', sort, order = 'asc' } = getFilter(
      el,
      attr
    );
  ...
  function findTarget(items, point) {
    return items[
      findIndex(items, item => pointInRect(point, item.getBoundingClientRect()))
    ];
  }
