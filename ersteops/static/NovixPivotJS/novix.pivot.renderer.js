...
$.pivotUtilities.novix_renderers = {
  'Pivot Table': function(pvtData, opts) {
    return $.pivotUtilities.novix_renderers['Input Table'](
      pvtData,
      $.extend({}, opts, { readOnly: true })
    );
  },
  ...
  'Input Table': function(pvtData, opts) {
    ...
    var parsePivotData = function(pvtData) {
      ...
      dataSource.fixedColumns =
        pvtData.rowAttrs.length + (hasColumnAttr ? 1 : 0);
      ...
      //columns
      var totalColumns = (dataSource.totalColumns =
        pvtData.colKeys.length +
        pvtData.rowAttrs.length +
      ...
      if (hasRowAttr && hasColumnAttr) {
        addMergeCell(
          0,
          0,
          hasColumnAttr ? pvtData.colAttrs.length : 1,
          hasRowAttr ? pvtData.rowAttrs.length : 1
        );
      }
      ...
      for (
        var nn = 0;
        nn < pvtData.colAttrs.length + (hasRowAttr ? 1 : 0);
        nn++
      ) {
        var rowData = [];
            ...
            if (rr == 0) {
              ...
              if (rowKeys[rr][cc] == mergeDic[cc].priorText) {
                ...
                if (cc < rowKeys[rr].length - 1) {
                  addMergeCell(
                    mergeDic[cc].row + pvtData.colAttrs.length + 1,
                    cc,
                    mergeDic[cc].toMerge,
                    1
                  );
      ...
      if (opts.showTotals || addExtraRow) {
        ...
        if (pvtData.colAttrs.length > 0) {
          addMergeCell(
            0,
            totalColumns - 1,
            pvtData.colAttrs.length + (hasRowAttr ? 1 : 0),
            1
          );
        }
      }
    ...
    var renderTable = function(pvtData, opts) {
      ...
      var genericRenderer = function(
        ...
        if (
          opts.showTotals &&
          (row == dataSource.totalRows - 1 ||
            col == dataSource.totalColumns - 1)
        ) {
          //total
          ...
          if (changes && changes.length >= 0) {
            ...
            for (var nn = 0; nn < changes.length; nn++) {
              if (changes[nn][2] != changes[nn][3]) {
                var pivotChange = getPivotChange(
                  changes[nn],
                  pvtData,
                  dataSource
                );
      ...
      setTimeout(function() {
        ...
        if (currentState) {
          if (currentState && currentState.lastRowChanged) {
            table.selectCell(
              currentState.lastRowChanged + 1,
              currentState.lasColChanged,
              currentState.lastRowChanged + 1,
              currentState.lasColChanged
            );
